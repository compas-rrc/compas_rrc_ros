...
def arguments_adapter(adapted_func=None, string_values=None, float_values=None):
    def arguments_adapter_decorator(func):
        ...
        def wrapper(*args, **kwargs):
            func.__arg_map__ = dict(
                string_values=string_values, float_values=float_values
            )
            opts = {}
...
class WebserviceInterfaceAdapter(object):
    ...
    def get_operation_mode(self):
        ...
        return {"string_values": (response["_embedded"]["_state"][0]["opmode"],)}

    def get_execution_state(self):
        response = self.ws.do_get("/rw/rapid/execution")
        return {"string_values": (response["_embedded"]["_state"][0]["ctrlexecstate"],)}
    ...
    def get_task_execution_state(self, task_name):
        ...
        return {"string_values": (response["_embedded"]["_state"][0]["excstate"],)}
    ...
    def set_signal(self, signal_name, value):
        ...
        if signal["type"] == "DO":
            ...
            raise Exception(
                "Unexpected signal type. Received={}".format(signal["type"])
            )
    ...
    def get_signal(self, signal_name):
        ...
        return {"float_values": (float(response["_embedded"]["_state"][0]["lvalue"]),)}
    ...
    def ensure_write_access(self):
        ...
        operation_mode = (
            result["string_values"][0] if len(result["string_values"]) else ""
        )
    ...
    def reset_program_pointer(self):
        ...
        try:
            print("Resetting program pointer")
            result = self.ws.do_post(path)
        except WebServiceRequestError as e:
            print("Got a WebServiceRequestError, CODE={}, Match={}".format(e.code, e.code == 400))
            if e.code == 400:
                raise WebServiceRequestError("The program pointer cannot be reset in the current state", code=e.code)
            else:
                raise e
        except Exception as e:
            print("Got a exception, Exception={}".format(str(e)))
            raise e
        return {}
    ...
    def execute_instruction(self, message):
        ...
        if hasattr(self, instruction):
            ...
            try:
                ...
                raise WebServiceInstructionError(
                    'No implemention found for instruction="{}"'.format(instruction)
                )
        ...
        return Message(
            ...
            feedback=result,
            string_values=return_strings,
            float_values=return_floats,
        )
...
class WebserviceInterface(EventEmitterMixin):
    ...
    def subscribe(self, data, callback):
        ...
        item = doc.find(
            query
        )  # there's more than one, but the first one is the correct one
        ws_url = item.attrib["href"]

        cookie = "ABBCX={}".format(self.session.cookies["ABBCX"])
        header = {
            "Cookie": cookie,
            "Authorization": self.auth.build_digest_header("GET", ws_url),
        }
    ...
    def _parse_response(self, response, format="json"):
        ...
        raise Exception(
            "WebService unexpected result: HTTP status code={}".format(
...
if __name__ == "__main__":
    ...
    m = Message(
        json.dumps(dict(path="/rw/motionsystem/mechunits/ROB_1/jointtarget")),
        feedback_level=1,
    )
    m.string_values = []
    m.float_values = []
